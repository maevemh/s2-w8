...
public class Forest {
    ...
    public void initializeForest(){
        ...
        while(count < initialTreeCount){
            ...
            if(grid[r][c].getState()==0 || grid[r][c].getState()==Tree.BURNING){
                grid[r][c].setState(1);
                grid[r][c].setBurnTime(0);
    ...
    public Tree[][] deepCopy(){
        ...
        for(int r=0; r<grid.length; r++){
            for(int c=0; c<grid[0].length; c++){
                copy[r][c].setState(grid[r][c].getState());
                copy[r][c].setBurnTime(grid[r][c].getBurnTime());
            }
        }
        return copy;
    }
